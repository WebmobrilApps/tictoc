...
class TicTocCubit extends Cubit<TicTocState> {
  ...
  Future<void> otherProfileCall1(String userId) async {
    emit(state.copyWith(status: TicTocStatus.getOtherProfileLoading1));
    try{
      ResponseData responseData = await repository.otherProfile(userId);
      emit(state.copyWith(status: TicTocStatus.getOtherProfileSuccess1,responseData: responseData));
    }
    on ErrorData catch (errorData){
      emit(state.copyWith(status: TicTocStatus.getOtherProfileError1,errorData: errorData,error: null));
    }
    catch(e){
      emit(state.copyWith(status: TicTocStatus.getOtherProfileError1,error: e.toString(),errorData: null));
    }
  }
  ...
  Future<void> saveLeaveReasonCall(Map<String, dynamic> logoutDetails) async {
    emit(state.copyWith(status: TicTocStatus.saveLeaveReasonLoading));
    try{
      ResponseData responseData = await repository.saveLeaveReason(logoutDetails);
      emit(state.copyWith(status: TicTocStatus.saveLeaveReasonSuccess,responseData: responseData));
    }
    on ErrorData catch (errorData){
      emit(state.copyWith(status: TicTocStatus.saveLeaveReasonError,errorData: errorData,error: null));
    }
    catch(e){
      emit(state.copyWith(status: TicTocStatus.saveLeaveReasonError,error: e.toString(),errorData: null));
    }
  }

  Future<void> guestLogoutCall() async {
    emit(state.copyWith(status: TicTocStatus.guestLogoutLoading));
    try{
      ResponseData responseData = await repository.guestLogout();
